...
class Runner:
    ...
    def __init__(self, env, is_collect_data = False, is_save_data = False, device='cpu'):
        from .ppo import PPO
        ...
        if is_collect_data:
            ...
            print('load parameters from dir : {}'.format(RunnerArgs.resume_path))
    ...
    def collect_data(self, num_learning_iterations, init_at_random_ep_len=False, eval_freq=100, curriculum_dump_freq=500, eval_expert=False):
        from ml_logger import logger
        # initialize writer
        ...
        for it in tqdm(range(self.current_learning_iteration, tot_iter)):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    # shape (num_env, obs shape) -> (iter_number, num_env, obs_shape)
        ...
        #shape (total_step, num_env, obs_shape) -> (num_env, total_step, obs_shape)
        ...
        with h5py.File('para_data_0809.hdf5', 'w') as f:
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            if it % RunnerArgs.save_interval == 0:
                with logger.Sync():
                    ...
                    logger.torch_save(self.alg.actor_critic.actor_body.state_dict(), f"checkpoints/actor_weights_{it:06d}.pt")
                    logger.duplicate(f"checkpoints/actor_weights_{it:06d}.pt", f"checkpoints/actor_weights_last.pt")

                    logger.torch_save(self.alg.actor_critic.std.data, f"checkpoints/std_weights_{it:06d}.pt")
                    logger.duplicate(f"checkpoints/std_weights_{it:06d}.pt", f"checkpoints/std_weights_last.pt")
