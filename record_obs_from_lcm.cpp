...
{
public:
    Custom(uint8_t level): safe(LeggedType::Aliengo), udp(LOCAL_PORT, TARGET_IP,TARGET_PORT, LOW_CMD_LENGTH, LOW_STATE_LENGTH){
        udp.InitCmdData(cmd);
    ...
    void handleRecorderLCM(const lcm::ReceiveBuffer *rbuf, const std::string & chan, const obs_action_lcmt * msg);
...
void Custom::init()
{
    ...
    _simpleLCM.subscribe("obs_action", &Custom::handleRecorderLCM, this);
...
void Custom::handleRecorderLCM(const lcm::ReceiveBuffer *rbuf, const std::string & chan, const obs_action_lcmt * msg){
    (void) rbuf;
    (void) chan;
...
void Custom::RobotControl()
{
    ...
    std::vector<float> states(obs_action.observation, obs_action.observation + sizeof(obs_action.observation) / sizeof(obs_action.observation[0]));
    std::vector<float> action(obs_action.action, obs_action.action + sizeof(obs_action.action) / sizeof(obs_action.action[0]));
    // if(!(states[0]) && !(states[5])){
    //     HDF5Recorder.finish_episode();
    // }else{
    //     HDF5Recorder.new_episode();
    // }
    // if (_keyData.btn.components.L2 && !rc_command.left_lower_left_switch){
    //     HDF5Recorder.finish_episode();
    //     // cout << "Press L2" << endl;
    // }
    // if ((abs(state.imu.rpy[0]) > 1.6) || (abs(state.imu.rpy[1]) > 1.6)){
    //     HDF5Recorder.finish_episode();
    // }else if(_keyData.btn.components.R2 && !rc_command.left_upper_switch){
    //     HDF5Recorder.new_episode();
    //     // cout << "Press R2" << endl;
    // }

    // T265_reader.appendPoseData(states);

    // std::cout << "x diff: " << states[3]-states[-6] << " y diff: " << states[4]-states[-5] << " z diff: " << states[5]-states[-4] << std::endl;
    // for(auto velue: states){
    //     std::cout<<velue<< "  ";
    // }
    // std::cout<<std::endl;
    // HDF5Recorder.record_step(action, states);
    if(counter >= 100){
    ...
    rc_command.left_stick[0] = _keyData.lx;//y speed
    rc_command.left_stick[1] = _keyData.ly;//x speed
    rc_command.right_stick[0] = _keyData.rx;//yaw speed
    rc_command.right_stick[1] = _keyData.ry;//z speed
