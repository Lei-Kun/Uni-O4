def train_go1(headless=True):
    ...
    Cfg.curriculum_thresholds.tracking_ang_vel = 1.5
    Cfg.curriculum_thresholds.tracking_lin_vel = 1.0
    ...
    Cfg.domain_rand.randomize_lag_timesteps = False  #TODO
    Cfg.control.control_type = "actuator_net"
    ...
    Cfg.commands.limit_vel_y = [-0.2, 0.2]
    Cfg.commands.limit_vel_yaw = [-2.0, 2.0]
    ...
    if is_collect_data:
        ...
        num_sample = 5e5 
        iter_num = int(num_sample/(24*Cfg.env.num_envs))
        runner.collect_data(num_learning_iterations=iter_num, init_at_random_ep_len=True, eval_freq=100)
    else:
      num_sample = 1.2e7
      is_save_data = True
      iter_num = int(num_sample/(24*Cfg.env.num_envs))
      runner = Runner(env, is_collect_data=is_collect_data, is_save_data=is_save_data, device=f"cuda:{gpu_id}")
      runner.learn(num_learning_iterations=iter_num, init_at_random_ep_len=True, eval_freq=100)
