...
class BehaviorProximalPolicyOptimization(ProximalPolicyOptimization):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            is_iql=is_iql,
            pi_activation_f=pi_activation_f)
...
class AdaptiveBehaviorProximalPolicyOptimization():
    def __init__(
        ...
        for _ in range(belief_dim):
            bppo = BehaviorProximalPolicyOptimization(device, state_dim, hidden_dim, depth, action_dim, policy_lr, clip_ratio, entropy_weight, decay, omega, batch_size, is_iql, pi_activation_f=pi_activation_f)
        ...
        for i in range(self._num_policy):
            ...
            if self._is_iql:
                ...
                if isinstance(Q, list): # fitted q_pi else q_bc
                    advantage = Q[i](s, action) - s_value
                else:
    ...
    def ope_dynamics_eval(self, args, dynamics_eval, q_eval, dynamics, eval_buffer, env, mean, std):
        best_mean_qs =  []
        for i, bppo in enumerate(self.bppo_ensemble):
            if isinstance(q_eval, list):
                best_mean_q, _ = dynamics_eval(args, bppo, q_eval[i], dynamics, eval_buffer, env, mean, std)
            else:
                best_mean_q, _ = dynamics_eval(args, bppo, q_eval, dynamics, eval_buffer, env, mean, std)
