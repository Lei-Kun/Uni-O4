...
{
public:
    Custom(uint8_t level): safe(LeggedType::Go1), udp(LOCAL_PORT, TARGET_IP,TARGET_PORT, HIGH_CMD_LENGTH, LOW_STATE_LENGTH){
        udp.InitCmdData(cmd);
    ...
    bool right_upper_switch_pressed = false;
};
...
void Custom::RobotControl()
{
    ...
    // right_upper_switch_pressed = (_keyData.btn.components.R1 && (!rc_command.right_upper_switch)) || right_upper_switch_pressed;
    if (_keyData.btn.components.R1 && !rc_command.right_upper_switch){
        // std::cout<<"Pressed"<<std::endl;
        HDF5Recorder.new_episode();
        right_upper_switch_pressed = false;
    }
    if (_keyData.btn.components.L1 && !rc_command.left_upper_switch){
        // std::cout<<"Pressed"<<std::endl;
        switch (cmd.mode){
            case 5://pos down
                cmd.mode = 6;//pos up
        ...
        std::cout<<"mode switch to: "<<cmd.mode<<endl;
    }
    ...
    rc_command.left_stick[0] = record_state(_keyData.lx);//y speed
    rc_command.left_stick[1] = record_state(_keyData.ly);//x speed
    rc_command.right_stick[0] = record_state(_keyData.rx, -1);//yaw speed
    rc_command.right_stick[1] = record_state(_keyData.ry);//z speed

    cmd.velocity[0] = rc_command.left_stick[1];
    cmd.velocity[1] = rc_command.left_stick[0];
    cmd.yawSpeed = -rc_command.right_stick[0];
    cmd.bodyHeight += 0.01 * rc_command.right_stick[1];
    ...
    // if(mode != cmd.mode){
    //     cmd.mode = mode;
    //     std::cout<<"mode switch to: "<<mode<<endl;
    // }
    ...
    for(int i = 0; i < 12; i++){
        ...
        // joint_state_simple.tau_est[i] = record_state(state.motorState[i].tauEst);
    ...
    T265_reader.appendPoseData(states_step);
...
void signalHandler( int signum ) {
    ...
    std::cout << "saving the last episode data\n";
    HDF5Recorder.close();
    exit(signum);  
}
int main(void)
