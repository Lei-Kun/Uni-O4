...
class ReplayBuffer:
    ...
    def load_dataset(self, dataset: Dict[str, np.ndarray]) -> None:
        observations = np.array(dataset["observations"][:-1, :], dtype=self.obs_dtype)
        next_observations = np.array(dataset["observations"][1:, :], dtype=self.obs_dtype)
        actions = np.array(dataset["actions"][:-1, :], dtype=self.action_dtype)
        rewards = np.array(dataset["rewards"], dtype=np.float32).reshape(-1, 1)[:-1, :]
        terminals = np.array(dataset["terminals"], dtype=np.float32).reshape(-1, 1)[:-1, :]
        ...
        self._ptr = len(observations) - 1 
        self._size = len(observations) - 1
    def state_normalize(self,)-> None:
        mean = self.observations.mean(0, keepdims=True)
        std = self.observations.std(0, keepdims=True) + 1e-10
