...
class ActorCritic(nn.Module):
    ...
    def deploy_sample(self, observation_history):
        latent = self.adaptation_module(observation_history)
        mean = self.actor_body(torch.cat((observation_history, latent), dim=-1))
        # log_std = soft_clamp(self.std, (-10., -2.))
        # std = log_std.exp()
        std = soft_clamp(self.std, (0., 0.8))
        self.distribution = Normal(mean, mean * 0. + std)

        return torch.clip(self.distribution.sample(), -100, 100)

    def act(self, observation_history, **kwargs):
        self.update_distribution(observation_history)
        return self.distribution.sample()
    def mean(self, observation_history, **kwargs):
        self.update_distribution(observation_history)
        return self.distribution.mean
