...
def termination_fn_neorl_halfcheetah(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def termination_fn_hopper(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def termination_fn_neorl_hopper(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def termination_fn_halfcheetahveljump(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def termination_fn_walker2d(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def termination_fn_neorl_walker2d(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def termination_fn_pen(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def terminaltion_fn_door(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def terminaltion_fn_hammer(obs, act, next_obs, env = None):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
    ...
    return done
def terminaltion_fn_antmaze(obs, act, next_obs, env = None):
        assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
